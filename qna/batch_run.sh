...
EXE_DIR=$SOFT_DIR/$BUILD_DIR/src
OUTPUT_DIR=${PROJECT_DIR}/outputs/$SETUP_SIM/$SETUP_REC/$PDG/set2/sgnl_1
# OUTPUT_DIR=${PROJECT_DIR}/outputs_psd/$SETUP_SIM/OlegSelection_sumw
...
ls -d /lustre/cbm/users/lubynets/weightadd/outputs/$SETUP_SIM/$SETUP_REC/$PDG/$INDEX/fillerOut.root > filelist_sec.list
fi
...
./QnAnalysisCorrect -i $FILELIST_DIR/filelist.$INDEX.list filelist_sec.list \
                    -t aTree aTree \
                    --yaml-config-file $YAML_DIR/correction-config.$PDG.${PBEAM}agev.psd.yml \
...
if [ $CORR_STEP = 2 ]
then
./QnAnalysisCorrelate --configuration-file $YAML_DIR/correction-config.$PDG.${PBEAM}agev.psd.yml \
